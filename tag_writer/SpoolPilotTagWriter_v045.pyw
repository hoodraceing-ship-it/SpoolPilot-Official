"""SpoolPilot Tag Writer 0.4.5 release entrypoint.

Keeps the managed Proxmark client and safe updater from 0.4.4 while making
Tk/Tkinter an explicit frozen dependency so the packaged Windows EXE can load
the dynamically-loaded UI core.
"""
...
# These imports are intentionally explicit.  The application core is loaded
# dynamically from an included .pyw file, so PyInstaller cannot discover its
# tkinter imports by normal static analysis.  Importing them here causes the
# tkinter hook and Tcl/Tk runtime files to be bundled into the Windows EXE.
...
def _validate_reader_selection(self):
    ...
    # The potentially large repair/download runs inside Pm3Runner.run on the
    # worker thread so the Tk UI stays responsive.
...
def _main() -> None:
    # This argument is used by GitHub Actions after PyInstaller builds the EXE.
    # Getting this far proves tkinter, the dynamic UI core, and client_support
    # were all present in the frozen application.
    ...
    # PYINSTALLER_RESET_ENVIRONMENT is only required while bootstrapping a new
    # frozen process. Do not pass it needlessly to normal child tools.

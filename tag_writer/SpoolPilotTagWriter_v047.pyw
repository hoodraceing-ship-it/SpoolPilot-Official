"""SpoolPilot Tag Writer 0.4.7 release entrypoint.

Fixes sector trailer programming with current Proxmark clients.  Current Iceman
protects strict read-only ACL trailer writes unless --force is explicitly used;
SpoolPilot now uses force only for the deliberate sector-protection write and
then verifies access with the selected filament keys before continuing.
"""
...
# PyInstaller 6.9+ restart handling.
...
# Current Iceman `hf 14a reader` reports identifiers rather than the literal
# words "MIFARE Classic 1K".  Normalize ATQA 00 04 + SAK 08 to the existing
# core's compatibility predicate.
...
# Current Proxmark protects trailer writes that establish strict read-only ACLs.
# This is exactly the intended Bambu-compatible sector protection operation, so
# force only this trailer write.  Do not force ordinary data or UID writes.
def _seal_sector_force_acl(self, sector: int) -> None:
    ...
    # A single forced trailer write is sufficient. Verify it repeatedly with
    # the new keys; if verification fails, stop before touching another sector.
    commands = [
        core.read_command(block, target_a, "A"),
        core.read_command(block, target_b, "B"),
        core.write_command(trailer_block, trailer, core.DEFAULT_KEY, "A", force=True),
        core.read_command(block, target_a, "A"),
        core.read_command(block, target_b, "B"),
        core.read_command(block, target_a, "A"),
        core.read_command(block, target_b, "B"),
    ...
    successful = core.successful_block_reads(
        core.parse_events(result.output), block, target_a, target_b
    )
    confirmed = next(
    ...
    if confirmed is None:
        raise RuntimeError(
            f"Sector {sector} protection could not be verified after the forced trailer write. "
            f"Recovery record: {self.recovery_file}"
        )

    _, key_type, key, _ = confirmed
    self.sectors[sector] = core.SectorState(sector, "target", key, key_type)
...
core.SafeWriter._seal_sector = _seal_sector_force_acl
...
def _main() -> None:
    if "--self-test" in sys.argv:
        ...
        forced = core.write_command(
            63,
            "2E00530F143087878769265199432E30",
            core.DEFAULT_KEY,
            "A",
            force=True,
        )
        if "--force" not in forced:
            raise RuntimeError("Sector trailer force flag regression")
        return
